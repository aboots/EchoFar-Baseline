...
class LingshuFinetuneCollator:
    ...
    def _find_response_start(self, input_ids: torch.Tensor) -> int:
        ...
        # Encode "assistant\n" as produced by the chat template
        ...
        # Fallback: mask everything (zero loss — signals a parsing failure)
...
def train(args: argparse.Namespace) -> None:
    ...
    collator = LingshuFinetuneCollator(processor, SYSTEM_PROMPT, PROMPT_SUFFIX)
    ...
    # --- Optimizer & scheduler (matching main_model.py style) ---
    ...
    print(
        f"\nTo run inference with the finetuned model, pass "
        f"--model_name_or_path {args.output_dir / 'checkpoint-best'} "
        f"to the inference scripts and load with PeftModel.from_pretrained()."
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Finetune Lingshu-7B with LoRA (frozen vision backbone)"
    )

    # Paths
    parser.add_argument(
        "--model_name_or_path", type=str, default="lingshu-medical-mllm/Lingshu-7B"
    )
    parser.add_argument("--output_dir", type=Path, default=Path("./lingshu_finetuned"))
