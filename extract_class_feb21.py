...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Extract concept-level labels from echo findings and export a wide CSV "
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to the input JSON/JSONL file. This can be either the raw findings "
            "records (with a 'findings' field) or an extracted concepts store file "
            "(with a 'concepts' field)."
        ),
    )
    parser.add_argument(
        "--input-kind",
        type=str,
        choices=("auto", "findings", "extracted", "vlm"),
        default="auto",
        help=(
            "How to interpret --input. 'findings' expects raw findings with regions; "
            "'extracted' expects records with a 'concepts' list; 'vlm' expects model "
            "predictions/ground_truth to be parsed; 'auto' detects based on the first record."
        ),
    )
    parser.add_argument(
        "--output-json",
        type=str,
        help=(
            "Path to write extracted concept store JSON. Used when --input-kind is "
            "'findings' (or auto-detected as findings)."
        ),
    )
    parser.add_argument(
        "--output-csv",
        type=str,
        help="Path to write the wide CSV file."
    )
    parser.add_argument(
        "--output-summary-csv",
        type=str,
        help=(
            "Path to write concept_name/class_label summary counts (unique echo_id)."
        ),
    )
    parser.add_argument(
        "--missing-value",
...
def load_json_records(input_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        return list(iter_records_from_loaded_json(data))
...
def iter_records_from_loaded_json(data: Any) -> Iterable[Dict[str, Any]]:
    ...
    for value in data.values():
        if isinstance(value, dict):
            if is_record_dict(value):
                yield value
        elif isinstance(value, list):
            for item in value:
                if isinstance(item, dict) and is_record_dict(item):
                    yield item


def is_record_dict(value: Mapping[str, Any]) -> bool:
    """Check if a dictionary looks like a single echo record."""
    has_id = any(k in value for k in ("echo_id", "exam_id", "study_id", "id"))
    has_payload = any(k in value for k in ("findings", "concepts", "prediction", "ground_truth"))
    return bool(has_id and has_payload)


def get_echo_id(record: Mapping[str, Any]) -> str:
    """Get the echo_id from a record, or a default placeholder."""
    eid = record.get("echo_id") or record.get("study_id") or record.get("id") or record.get("exam_id")
    return str(eid) if eid is not None else "unknown"
...
def parse_diastolic_grade(normalized_sentence: str) -> Optional[int]:
    """Parse diastolic grade (I/II/III/IV or 1/2/3/4).

    Args:
        normalized_sentence (str): Normalized sentence.

    Returns:
        Optional[int]: Grade number if found.
    """
...
def classify_chamber_size(sentence: str, rng: random.Random) -> Optional[str]:
    """Classify chamber size (C1) from a sentence.

    Args:
        sentence (str): Sentence text.
        rng (random.Random): RNG for resolving ranges.

    Returns:
        Optional[str]: Class label or None.
    """
...
def classify_valve_leaflet_thickening(sentence: str, rng: random.Random) -> Optional[str]:
    """Classify valve leaflet thickening (C7) from a sentence.
...
def classify_pulmonary_artery_systolic_pressure(
    sentence: str,
    rng: random.Random,
) -> Optional[str]:
    """Classify pulmonary artery systolic pressure (C16) from a sentence.

    Args:
        sentence (str): Sentence text.
        rng (random.Random): RNG (unused).

    Returns:
        Optional[str]: Class label (normal/elevated) or None.
    """
    text = normalize_text(sentence)
    if "pulmonary artery systolic pressure" not in text and "pasp" not in text:
        return None

    if (
        "elevated pulmonary artery systolic pressure" in text
        or re.search(
            r"\b(pulmonary artery systolic pressure|pasp)\b.{0,25}\b(elevated|increase|increased|high)\b",
            text,
        )
        is not None
        or re.search(
            r"\b(elevated|increase|increased|high)\b.{0,25}\b(pulmonary artery systolic pressure|pasp)\b",
            text,
        )
        is not None
    ):
    ...
    if (
        "normal pulmonary artery systolic pressure" in text
        or re.search(
            r"\b(pulmonary artery systolic pressure|pasp)\b.{0,25}\bnormal\b",
            text,
        )
        is not None
        or re.search(
            r"\bnormal\b.{0,25}\b(pulmonary artery systolic pressure|pasp)\b",
            text,
        )
        is not None
    ):
...
def classify_sinuses_of_valsalva(
    sentence: str,
    rng: random.Random,
) -> Optional[str]:
    """Classify sinuses of Valsalva size (C12) from a sentence.
    ...
    Returns:
        Optional[str]: Class label (normal/dilated) or None.
    """
    text = normalize_text(sentence)
    if re.search(r"\bsinus(?:es)? of valsalva\b", text) is None:
        return None

    if (
        re.search(r"\bnot\b.{0,25}\b(dilat\w*|enlarg\w*)\b", text) is not None
        or re.search(r"\bno\b.{0,25}\b(dilat\w*|enlarg\w*)\b", text) is not None
        or re.search(r"\bwithout\b.{0,25}\b(dilat\w*|enlarg\w*)\b", text) is not None
    ):
        return "normal"

    if re.search(r"\b(dilat\w*|enlarg\w*)\b", text) is not None:
        return "dilated"

    if re.search(r"\bnormal\b", text) is not None:
        return "normal"

    return None


def classify_ascending_aorta(
    sentence: str,
    rng: random.Random,
) -> Optional[str]:
    text = normalize_text(sentence)
    if "ascending aorta" not in text:
        return None
...
def classify_inferior_vena_cava(
    sentence: str,
    rng: random.Random,
) -> Optional[str]:
    text = normalize_text(sentence)
    if "inferior vena cava" not in text and "ivc" not in text:
        return None
    ...
    if "normal size" in text or (
        re.search(r"\bnormal\b", text) is not None and re.search(r"\bsize\b", text) is not None
    ):
        return "normal"
...
def classify_pericardial_effusion(
    sentence: str,
    rng: random.Random,
) -> Optional[str]:
    """Classify pericardial effusion presence (C14) from a sentence.
    ...
    Returns:
        Optional[str]: Class label (normal/abnormal) or None.
    """
    text = normalize_text(sentence)
    if "pericardial effusion" not in text:
        return None

    negated = (
        "no pericardial effusion" in text
        or "without pericardial effusion" in text
        or "absent pericardial effusion" in text
        or "no evidence of pericardial effusion" in text
        or re.search(r"\bnot\b.{0,25}\bpericardial effusion\b", text) is not None
        or re.search(r"\bno\b.{0,25}\bpericardial effusion\b", text) is not None
        or re.search(r"\bwithout\b.{0,25}\bpericardial effusion\b", text) is not None
    )
    if negated:
        return "normal"
...
def build_region_specs() -> List[RegionSpec]:
    """Build the region specifications and concept rules.

    Returns:
        List[RegionSpec]: Region specs.
    """
    ...
    pulmonary_valve_artery = RegionSpec(
        ...
        rules=(
            ...
            ConceptRule(
                ...
                "C16",
                classify_pulmonary_artery_systolic_pressure,
            ),
        ),
    )

    aorta = RegionSpec(
        canonical_name="Aorta",
        a_label="A9",
        prefix="Aorta",
        rules=(
            ConceptRule("sinuses_of_valsalva", "C12", classify_sinuses_of_valsalva),
            ConceptRule("ascending_aorta", "C13", classify_ascending_aorta),
        ),
    )

    venous = RegionSpec(
        canonical_name="Venous",
        a_label="A10",
        prefix="Venous",
        rules=(ConceptRule("inferior_vena_cava", "C15", classify_inferior_vena_cava),),
    )

    pericardium_other = RegionSpec(
        canonical_name="Pericardium/Other",
        a_label="A11",
        prefix="PericardiumOther",
        rules=(ConceptRule("pericardial_effusion", "C14", classify_pericardial_effusion),),
    )
...
def build_region_aliases() -> Dict[str, List[str]]:
    return {
        ...
        "Aorta": ["Aorta", "Aortic root", "Aortic Root"],
        "Venous": ["Venous", "Inferior vena cava", "Inferior Vena Cava", "IVC"],
        "Pericardium/Other": ["Pericardium/Other", "Pericardium / Other", "Pericardium"],
    }
...
def format_stream_id(stream_id: Any, missing_value: str) -> str:
    """Format stream_id for CSV output.

    Args:
        stream_id (Any): Raw stream_id value.
        missing_value (str): Placeholder for missing values.

    Returns:
        str: A CSV-friendly string representation.
    """
...
def build_wide_rows_and_distribution(
    ...
    for record in extracted_records:
        ...
        row: Dict[str, str] = {
            "echo_id": echo_id_str,
            "stream_id": format_stream_id(stream_id, missing_value),
        }
...
def write_wide_csv(
    ...
    fieldnames = ["echo_id", "stream_id", *concept_columns]
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def detect_input_kind(records: Sequence[Mapping[str, Any]]) -> str:
    """Detect the format of the input records."""
    for record in records:
        if not isinstance(record, dict):
            continue
        # Check for extracted concepts first
        if "concepts" in record:
            return "extracted"
        # Check for intermediate findings
        if "findings" in record:
            return "findings"
        # Check for raw VLM outputs
        if "prediction" in record or "ground_truth" in record:
            return "vlm"
    return "unknown"
...
def run(
    ...
    detected_kind = detect_input_kind(records) if input_kind == "auto" else input_kind
    print(f"Detected input kind: {detected_kind}")
    
    region_specs = build_region_specs()

    # Step 1: If raw VLM, normalize and parse into findings
    if detected_kind == "vlm":
        processed_records = []
        for record in records:
            text = record.get("ground_truth" if use_gt else "prediction", "")
            if not no_normalize:
                text = normalize_vlm_prediction(str(text))
        ...
        detected_kind = "findings"

    # Step 2: Extract concepts from findings
    if detected_kind == "findings":
        extracted_records = process_records(records, region_specs=region_specs, seed=seed)
        if output_json_path:
            write_json(output_json_path, extracted_records)
    elif detected_kind == "extracted":
        extracted_records = list(records)
    else:
        raise SystemExit(f"Unsupported input format: {detected_kind}. Records must contain 'prediction', 'findings', or 'concepts'.")

    # Step 3: Tabulate and summarize
